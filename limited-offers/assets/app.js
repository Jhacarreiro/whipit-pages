...
function hoursFor(store){const items=(store.opening_hours&&store.opening_hours.items)||[];if(!items.length)return null;const now=new Date(),today=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')}`;const exact=items.find(x=>x.date===today);if(exact)return {item:exact,from:today,exact:true};const wd=new Date(`${today}T00:00:00`).getDay();const sameWeekday=items.filter(x=>x.date&&new Date(`${x.date}T00:00:00`).getDay()===wd).sort((a,b)=>a.date<b.date?1:-1);if(sameWeekday.length)return {item:sameWeekday[0],from:sameWeekday[0].date,exact:false};return null}
function formatRanges(item){const ranges=item.timeRanges||[];if(!ranges.length)return tr('closed','Closed');return ranges.map(r=>`${(r.from||'').slice(11,16)}–${(r.to||'').slice(11,16)}`).join(', ')}
function todayHours(store){const hit=hoursFor(store);if(!hit)return tr('hours_unknown','Hours n/a');return formatRanges(hit.item)}
function hoursProvenance(store){const hit=hoursFor(store);if(!hit||hit.exact)return '';return `${tr('hours_from','Hours from')} ${hit.from}`}
